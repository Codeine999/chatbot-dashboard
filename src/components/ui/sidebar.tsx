...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    if (isMobile) {
      ...
      localStorage.setItem('sidebarMobileOpen', JSON.stringify(newState))
    } else {
      const newState = !open
      setOpen(newState)
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-slot="sidebar"
          data-mobile="true"
          className="bg-sidebar text-sidebar-foreground w-(--sidebar-width)"
          style={
            {
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <PanelLeftIcon className="!w-6 !h-8 text-[#6f7d9a]" />
